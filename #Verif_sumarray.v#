...
(** The [ident*funspec] begins, [DECLARE _sumarray].  The Coq name
 ...
 Definition of [_sumarray] near the beginning of [sumarray.v].  Later
 in [sumarray.v], you can see [Definition f_sumarray] that is the
 C-language function body (represented as a syntax tree), and even
 later, near the end of the [Definition global_definitions] you can
 see the binding of [_sumarray] to [f_sumarray].
  ...
  (in this case [_a, _n]) and the postcondition is parameterized by a
  identifier [ret_temp], which is short for, ``the temporary variable
  ...
  The [LOCAL] propositions [Q] are _variable bindings_ of type
  [localdef].  Here, the function-parameters [a] and [n] are treated
  as nonaddressable local variables, or ``temp'' variables.  The
  localdef [temp _a a] says that (in this program state) the contents
  of C local variable [_a] is the Coq value [a].  
...
(** The [SEP] conjuncts [R] are _spatial assertions_ in separation
  logic.  In this case, there's just one, a [data_at] assertion saying
...
>> 
  ...
  The [LOCAL] clause _must not mention_ the function parameters or
  ...
  function's caller).  The postcondition's [LOCAL] will only mention 
  the return-temporary [ret_temp].
...
(** The function-spec for [main] has a special form, which we discuss
   below at the heading, _Global variables and main_ .  In particular,
   its precondition is defined using [main_pre]. *)
...
(** *** Integer overflow. 
 ...
 needs that [0 <= x <= Int.max_unsigned] or 
...
Print Gprog.  (* Quite long, and not interesting, but note that the last
  bindings in the list are the two [DECLARE]s for [_sumarray] and [_main]. *)
...
(** We do Hoare logic proof by forward symbolic execution.
    ...
    a Hoare triple about the statement [(i=0; ...more commands...)].
    In a forward Hoare logic proof of [{P}(i=0;...more...){R}] 
...
(** Look at the precondition of the current proof goal, that is,
  ...
  the _postcondition_ of [i=0;].  It's much like the
  _precondition_ of [i=0;] except for one change:  we now know that [i]
  is equal to [0], which is expressed in the [LOCAL] part as 

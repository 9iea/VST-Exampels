...
-
  ...
  entailer!.
  simplify_value_fits in H4. simplify_value_fits in H1. simplify_value_fits in H1.
  clear Delta. assert_PROP (q<>nullval).
  +
    entailer!.  
  +
    rewrite if_false by auto. entailer!.
-
  forward. unfold stack. Exists y. entailer!. 
  Qed.
...
forward_if (vret<>nullval).
  -
    ...
    +
      entailer!.
  -
    subst vret.  rewrite if_true by auto. forward_call (tt). contradiction.
  -
    rewrite if_false by auto. forward. rewrite if_false by auto. entailer!.
  -
    Intros. rewrite if_false by auto. Intros. forward. forward. forward. forward. forward.
    Exists vret. cancel. unfold listrep. Exists q. cancel.
  Qed. 
...
Proof.
  ...
  -
    rewrite if_true.
    +
      subst vret. forward_call (tt). contradiction.
    +
      auto.
  - 
    rewrite if_false by auto. forward. entailer!.  rewrite if_false by auto. entailer!. 
  -
    Intros. rewrite if_false by auto. Intros. forward. forward. Exists vret. entailer!. Exists nullval.

...
- (* If-else *)
 ...
 + 
      Exists (@nil val) h r x u.
      subst s1. entailer!. unfold nt_lseg; entailer!.
 +
      entailer!.
 +  clear h r u H0; rename u0 into u.
    rewrite (listrep_nonnull _ u) by auto.
    Intros c s1d z. Search nt_lseg. 
    subst LOOP_BODY.
    forward. 
    forward.  
    Exists ((s1a ++ b :: nil),c,s1d,u,z). unfold fst, snd.
    ...
    Search nt_lseg.
    sep_apply (singleton_nt_lseg').
    entailer.
    rewrite sepcon_assoc.
    sep_apply (pull_left_special0 (nt_lseg s1a x t)).
    entailer.
    sep_apply (nt_lseg_nt_lseg).
    entailer!.
 +
   clear h r u H0; rename u0 into u.
   rewrite (listrep_null s1c) by auto.

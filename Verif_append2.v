...
(** Separating implication is another separation logic operator. 
  ...
  it is usually called "magic wand". The following [Locate] command 
  and [Check] command show this notation definition and its typing
  information. *)
...
(** In separation logic, a heaplet (piece of memory) [m] satisfies [P -* Q]
  if and only if: for any possible heaplet [n], 
  if [n] and [m] are disjoint and [n] satisfies [P], then the
  combination of [n] and [m] will satisfy [Q].
...
(** In the following exercise, prove [wand_derives] using 
  [wand_sepcon_adjoint] and [modus_ponens_wand]. You can use other proof 
  rules about [*], such as [sepcon_derives].
  Also, the tactic [sep_apply] may be useful. *)
...
Lemma wand_derives_from_adjoint_and_modus_ponens:
  forall P P' Q Q' : mpred, 
   P' |-- P  ->  Q |-- Q'  ->  P -* Q |-- P' -* Q'.
Proof.
  intros. rewrite <- wand_sepcon_adjoint. rewrite pull_left_special0.
  sep_apply H.
...
(** Theorem [wand_frame_ver] is the counterpart of implication's
...
Proof.
  intros. 
  unfold wlseg. fold wlseg. apply allp_right; intros v.
  simpl.   
  apply  wand_sepcon_adjoint. 
  unfold listrep.  
  fold listrep. Exists y.
  entailer!.   
Qed.
(** [] *)
...
Proof.  
  ...
  -
    intros. rewrite  wand_sepcon_adjoint.  
    unfold wlseg. autorewrite with sublist. apply (allp_left _ s2).
    simpl.  
    entailer!.
  -
    simpl. intros. rewrite wand_sepcon_adjoint.
    unfold wlseg. apply (allp_left _ s2). simpl. entailer!.
Qed.
(** [] *)
...
-
  ...
  forward_while
    (EX s1a: list val, EX b: val, EX s1c: list val, EX t: val, EX u: val,
       ...
       SEP (wlseg s1a x t;
            ...
            listrep s2 y))%assert.
  +
    Exists (@nil val) h r x u. subst s1. entailer!. unfold wlseg.
    apply allp_right; intros v. Search wand.
  ...
  +
    ...
    *
      ...
      simpl. reflexivity.
    *
       sep_apply (singleton_wlseg). 
       sep_apply wlseg_wlseg. entailer!.
  +
       (* loop body preserves invariant *)
   (** Step forward through the loop body; along the way you'll need
      to do other transformations on the current assertion
      in order to uncover opportunities to step [forward].
      At the end of the loop body, you need to prove that a list
      segment for [s1a] and a singleton cell for [b] forms a longer list
...
modifies the sub-memory described by that [wlseg] predicate. In separation
...
Proof.
  intros.
  unfold lbseg. apply (allp_right); intros v.
...
Proof.
  ...
  -
    intros. rewrite  wand_sepcon_adjoint.  
    unfold lbseg. autorewrite with sublist. apply (allp_left _ s2).
    simpl.  
    entailer!.
  -
    simpl. intros. rewrite wand_sepcon_adjoint.
    unfold lbseg. apply (allp_left _ s2). simpl. entailer!.
Qed.
...
(** However, [wlseg] does not derive [lseg] in general. As mentioned
  ...
  [data_at_ Tsh t_list y |-/- lseg [a] x y]. But on the hand, 

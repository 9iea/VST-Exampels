...
(** This [data_at Tsh tschar (Vint (Int.repr 72)) p] is an [mpred],
   that is, a memory predicate in separation logic.  It says, at
   address [p] in memory there is a sequence of bytes whose length
   is appropriate for type [tschar];  that is, one byte.  The
   ...
   memory at address [p] is the "top share" [Tsh] *)

Check (data_at Tsh tschar (Vbyte (Byte.repr 72))).
(** We can express the same thing using [Vbyte (Byte.repr 72)].
    In fact, [Vbyte] is not a primitive CompCert value,
    it is a definition: *)
...
(** This says, no element of the list [Hello'] is equal to [Byte.zero].
    In memory at address [p] there is an array of 6 bytes, whose
    contents are the contents of [Hello'] with a [Byte.zero] appended
    at the end. *)

(** Sometimes we know that there is a null-terminated string inside an
    array of length [n].  That is, there are [k] nonnull characters
    (where [k<n]), followed by a null character, followed by [n-(k+1)]
    uninitialized (or don't-care) characters.  We represent this with
    the [cstringn] predicate. *)
Print cstringn. (* =
...
(** Now we are at a for-loop.  Unlike a simple while-loop, a for-loop may:
 ...
 The general form of Verifiable C's loop tactic is:
  ...
  In many cases the [forward_loop] tactic can figure out that
  the [continue:] invariant is not needed (if the loop doesn't contain
  a [continue] statement), or the [break:] postcondition is not needed
  (if there's no [break] statement, or if there are no commands after
  the loop).
...
(** This installation of Verifiable C is configured for 32-bit
   pointers.  But either way, to turn a [Ptrofs.int] value into a
   CompCert [val], we have [Vptrofs]. *)
Print Vptrofs. (* = 
 fun n : ptrofs =>
...
assert (Example: forall n, Vptrofs (Ptrofs.repr n) = Vint (Int.repr n)). {
  (* This assertion holds in a 32-bit C system. *)
  intros.
  hint.
...
*
  unfold Zlength. unfold map.
fold map. entailer!. autorewrite with sublist in *|-.  forward.
...
(** **** Exercise: 2 stars, standard (strcpy_then_clause)  
   ...
   proof of the [strcpy] function (in the _then_ clause of the _if_
   statement). *)
...
replace (n-i) with (1 + (n-(i+1))) by omega.
rewrite <- list_repeat_app' by omega.
rewrite !app_ass.
f_equal.
f_equal.
...
+ (* Prove the loop body 
    ...
    In the [then] clause of this proof, you may reach a proof goal
  similar to Lemma strcpy_then_clause, but not identical.  In that
  case you may want to use the same proof techniques you used in that
  exercise.  Ditto for the else clause, and strcpy_else_clause. *)
(* FILL IN HERE *) Admitted.
(** [] *)
...
Definition example_call_strcpy_spec :=
 DECLARE _example_call_strcpy
  ...
  POST [ tptr tschar ]
    PROP ()
    LOCAL (temp ret_temp (Vint (Int.repr (Z.of_N (Ascii.N_of_ascii "H"%char)))))
    SEP (cstring Ews (Hello' ++ [Byte.zero]) (gv ___stringlit_1)).

Lemma body_example_call_strcpy: semax_body Vprog Gprog f_example_call_strcpy example_call_strcpy_spec.
Proof.
start_function.
(** This proof is fairly straightforward.
  ...
  see how the [data_at] and [cstring] conjuncts must match your
  current assertion.

  Second, after the forward_call, don't forget to unfold [cstringn];
